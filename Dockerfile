...
# BASE IMAGE
# ===========================================
FROM docker-virtual-asa.asa.com.br/python:3.11-slim AS base
...
# Acquire::https::Verify-Peer resolve o erro de certificado SSL (Zscaler)
...
Trusted: yes\n" > /etc/apt/sources.list.d/jfrog.sources && \
    echo 'Acquire::https::Verify-Peer "false";\nAcquire::https::Verify-Host "false";' > /etc/apt/apt.conf.d/99no-ssl-verify
