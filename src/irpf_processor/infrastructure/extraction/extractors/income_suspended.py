...
class IncomeSuspendedHolderExtractor(ISectionExtractor):
    ...
    def _extract_from_page(self, page_text: str, page_num: int, seen_ids: set) -> list[dict]:
        ...
        for i, line in enumerate(lines):
            ...
            if not in_section:
                # Padrão 1: Marker completo - verificar se é TITULAR e não DEPENDENTES
                ...
                # Padrão 3: Detectar seção por "IMPOSTO COM EXIGIBILIDADE SUSPENSA" após título do titular
                # Verificar linhas anteriores para contexto
                if (
                    "EXIGIBILIDADE SUSPENSA" in upper_line
                    or "IMPOSTO COM EXIGIBILIDADE" in upper_line
                ):
                    # Verificar se nas linhas anteriores há indicação de TITULAR
                    for j in range(max(0, i - 5), i):
                        prev_line = lines[j].upper()
                        if self.HOLDER_MARKER in prev_line and "PELOS DEPENDENTES" not in prev_line:
                            in_section = True
                            break
        ...
        if pattern_alt:
            ...
            for j in range(idx + 1, min(idx + 5, len(lines))):
                ...
                cnpj_match = re.search(
                    r"(\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2}|\d{3}\.\d{3}\.\d{3}-\d{2})", next_line
                )
                if cnpj_match:
    ...
    def _calculate_totals(self, items: list[dict], pdf_totals: list[float] | None = None) -> dict:
        """Calcula totais e valida contra os totais do PDF."""
        pdf_totals = pdf_totals or []

        sum_income = round(
            sum(i.get("taxable_income_with_suspended_requirements", 0) for i in items), 2
        )
        ...
        return {
            "taxable_income_with_suspended_requirements": create_validated_total(
                sum_income, pdf_income
            ),
