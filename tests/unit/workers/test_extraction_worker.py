...
class TestShouldFallbackToOcr:

    def test_no_text_triggers_fallback(self):
        from irpf_processor.presentation.workers.extraction_worker import _should_fallback_to_ocr
        result = _should_fallback_to_ocr(
            has_text=False,
            had_timeout=False,
            extraction_warnings=[],
            pages_text={},
    ...
    def test_process_timeout_triggers_fallback(self):
        from irpf_processor.presentation.workers.extraction_worker import _should_fallback_to_ocr
        result = _should_fallback_to_ocr(
            has_text=True,
            had_timeout=True,
            extraction_warnings=["PROCESS_TIMEOUT: Extração total excedeu 300s"],
            pages_text={1: "some text here"},
            total_pages=1,
        )
        assert result is True

    def test_pdf_open_timeout_triggers_fallback(self):
        from irpf_processor.presentation.workers.extraction_worker import _should_fallback_to_ocr
        result = _should_fallback_to_ocr(
            has_text=False,
            had_timeout=True,
            extraction_warnings=["PDF_OPEN_TIMEOUT: pdfplumber.open() excedeu 60s"],
            pages_text={},
            total_pages=0,
        )
        assert result is True
    ...
    def test_insufficient_chars_triggers_fallback(self):
        from irpf_processor.presentation.workers.extraction_worker import _should_fallback_to_ocr
        result = _should_fallback_to_ocr(
            has_text=True,
            had_timeout=False,
    ...
    def test_low_page_text_ratio_triggers_fallback(self):
        from irpf_processor.presentation.workers.extraction_worker import _should_fallback_to_ocr
    ...
    def test_normal_text_does_not_trigger_fallback(self):
        from irpf_processor.presentation.workers.extraction_worker import _should_fallback_to_ocr
        pages = {i: "A" * 500 for i in range(1, 6)}
        result = _should_fallback_to_ocr(
            has_text=True,
            had_timeout=False,
            extraction_warnings=[],
    ...
    def test_partial_text_ok_if_enough_ratio(self):
        from irpf_processor.presentation.workers.extraction_worker import _should_fallback_to_ocr
        pages = {1: "A" * 500, 2: "B" * 500, 3: ""}
        result = _should_fallback_to_ocr(
            has_text=True,
            had_timeout=False,
            extraction_warnings=[],
...
class TestGetFallbackReason:

    def test_process_timeout_reason(self):
        from irpf_processor.presentation.workers.extraction_worker import _get_fallback_reason
        reason = _get_fallback_reason(
            has_text=False,
            had_timeout=True,
            extraction_warnings=["PROCESS_TIMEOUT: ..."],
        )
        assert reason == "process_timeout"

    def test_pdf_open_timeout_reason(self):
        from irpf_processor.presentation.workers.extraction_worker import _get_fallback_reason
        reason = _get_fallback_reason(
            has_text=False,
            had_timeout=True,
            extraction_warnings=["PDF_OPEN_TIMEOUT: ..."],
        )
        assert reason == "pdf_open_timeout"

    def test_no_text_reason(self):
        from irpf_processor.presentation.workers.extraction_worker import _get_fallback_reason
        reason = _get_fallback_reason(
            has_text=False,
            had_timeout=False,
        ...
        assert reason == "no_text_extracted"

    def test_insufficient_text_reason(self):
        from irpf_processor.presentation.workers.extraction_worker import _get_fallback_reason
        reason = _get_fallback_reason(
            has_text=True,
            had_timeout=False,
