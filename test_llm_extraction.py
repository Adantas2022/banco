...
This script demonstrates the simplified workflow with IRPFParser's
automatic LLM detection and fallback to regex extraction:
1. Load PDF (test_digital.pdf)
2. Use IRPFParser.parse() which automatically:
   - Creates ExtractionContext
   - Detects AssetsExtractor has LLM = True flag
   - Calls extract_with_llm() via asyncio.run()
   - Falls back to extract() if LLM fails
3. Display comprehensive results

IRPFParser handles all the complexity - this is the recommended approach.
"""
...
def test_llm_extraction_with_parser():
    """Test LLM extraction using IRPFParser with test_digital.pdf file."""
    
    print("\n" + "="*70)
    print("🧪 LLM EXTRACTION TEST - Using IRPFParser")
    print("="*70)
    
    # Find test_digital.pdf
    pdf_path = Path(__file__).parent / "test_digital.pdf"
    
    if not pdf_path.exists():
        # Try alternative paths
        possible_paths = [
            Path(__file__).parent / "test_digital.pdf",
            Path(__file__).parent / "exemplos" / "test_digital.pdf",
            Path(__file__).parent / "exemplos" / "GENESIS" / "test_digital.pdf",
        ]
        ...
        for alt_path in possible_paths:
            ...
            print(f"\n❌ ERROR: Could not find test_digital.pdf or test_digital.pdf")
            print(f"Looked in:")
    ...
    try:
        ...
        print("   3. Call AssetsExtractor via _run_extractor()")
        print("   4. Check LLM = True flag → calls extract_with_llm()")
        ...
        if result.warnings:
            ...
            # Check for LLM extraction info
            llm_warnings = [w for w in result.warnings if "[LLM]" in w]
            if llm_warnings:
        ...
        if assets:
            ...
            # Check if using regex instead of LLM
            if assets.get('extraction_method') != 'llm' and assets.get('extraction_method') != 'LLM':
                print("\n⚠️  Using regex extraction instead of LLM")
                print("   Check the warnings above for LLM errors")
        ...
        print("\nThis error occurs when LLM extraction tries to import SimpleIRProvider.")
        print("The parser will automatically fall back to regex extraction.")
        print("\nTo fix this, ensure Python path is correctly set and all dependencies are installed:")
        print("  pip install -e .")
